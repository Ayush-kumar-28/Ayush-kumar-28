Hi, I'm Ayush Kumar

🚀 Full-Stack Developer | AI Enthusiast | Open Source Contributor

🎓 B.Tech CSE @ Usha Martin University (2023–2027)

💡 Passionate about building scalable web apps & AI-powered systems

🏆 Top 10% on LeetCode (Rating: 1778)

🌟 Contributor @ GSSOC 2024

🥇 College Hackathon Winner (2025)

🛠️ Tech Stack:

💻 Languages

C / C++

Java

JavaScript / TypeScript

SQL

HTML / CSS

⚙️ Frameworks & Libraries:

React.js, Next.js

Node.js, Express.js

Tailwind CSS, Material UI, ShadCN

Redux Toolkit

☁️ Cloud & Databases:

MongoDB, MySQL, Redis

AWS (EC2, DynamoDB, Cognito)

Cloudinary, ConvexDB

🧰 Tools & DevOps:

Git & GitHub

Docker

CI/CD

Postman, ThunderClient

VS Code

🚀 Projects

🔹 MentorVerse

AI-powered full-stack mentorship platform

🤝 Connects mentors & mentees using intelligent matching

🔐 JWT-based authentication & protected routes

📅 Session booking & scheduling system

📹 Real-time video calling (Jitsi API)

Tech Used:
React.js, Node.js, Express.js, MongoDB, Tailwind CSS


🔹 AI-Powered Finance Tracker
Full-stack + ML-integrated finance management system

📊 Classifies transactions automatically

📈 Analyzes spending behavior

💡 Provides personalized investment recommendations

⚡ Built with microservices (FastAPI + Node.js)

Tech Used:
React.js, TypeScript, Node.js, Express.js, MongoDB, APIs


💼 Experience
Internshala Student Partner (ISP)
📅 Feb 2024 – Mar 2024

Promoted 30+ internship & training programs
Helped 60+ students register
Built strong communication & leadership skills

🏆 Achievements:

🥇 College Hackathon Winner (2025)
📊 Top 10% on LeetCode (Rating: 1778)
🧑‍💻 GSSOC 2024 Contributor
🏁 Participant – Flipkart Grid 6.0

📚 Relevant Coursework

Data Structures & Algorithms

DBMS

Operating Systems

OOPs

Computer Networks

Software Engineering

📫 Connect With Me

📧 Email: ayushkumar93945@gmail.com
💼 LinkedIn: https://linkedin.com/in/Ayushkumar
💻 GitHub: https://github.com/Ayush-kumar-28
🌐 Portfolio: 

⚡ Fun Fact

I love solving real-world problems using AI + Full Stack 🚀
